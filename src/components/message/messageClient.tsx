...
type ChatProfile = {
  id: string;
  display_name: string | null; // null 허용
  avatar_url: string | null; // null 허용
...
export default function MessageClient({
  ...
  const [me] = useState<string>(initialUserId);
  ...
  // ✅ 수정 코드
  const [peer, setPeer] = useState<ChatProfile | null>(initialProfile ?? null);
      ...
      <div className="lg:max-w-250 mx-auto">
        <div className="bg-white/40 rounded-xl shadow-md lg:shadow-xl h-200 lg:min-w-50 flex flex-row">
          ...
          >
            ...
            {/* 입력창 */}
            <div className="p-4">
              <div className="flex items-center gap-3 border border-[#E5E5EC] rounded-lg p-3">
                <input
                  ref={inputRef}
                  value={draft}
                ...
                {/* ✅ [수정됨] onClick={() => sendMessage} -> onClick={() => sendMessage()} */}
                <button
                  type="button"
                  onClick={() => sendMessage()}
