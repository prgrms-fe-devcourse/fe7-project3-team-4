...
/**
 * 사용자가 특정 뱃지를 착용하도록 프로필을 업데이트하는 Server Action입니다.
 * @param badgeId - 착용할 뱃지의 UUID (string)
 */
export async function equipBadgeAction(badgeId: string | null) {
  ...
  if (error) {
    console.error("뱃지 착용 실패:", error);
    return { error: "뱃지 착용 중 데이터베이스 오류가 발생했습니다." };
  }
...
export async function buyBadgeAction(badgeId: string) {
  ...
  // 이 RPC 함수는 DB 내부에서 RLS를 무시하고 포인트 차감 및 뱃지 지급을 하나의 트랜잭션으로 처리합니다.
  const { error } = await supabase.rpc("buy_badge", {
    badge_id_to_buy: badgeId,
  });

  if (error) {
    console.error("뱃지 구매 RPC 호출 실패:", error);
  ...
  // 구매 성공 후, 뱃지 상점 페이지('/store')의 서버 캐시를 무효화합니다.
  revalidatePath("/store");
