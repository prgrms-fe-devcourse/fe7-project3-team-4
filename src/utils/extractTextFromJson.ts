...
// [✅ 추가] Post Content(JSON)에서 PromptInput과 PromptResult 값을 동적으로 추출합니다.
export function extractPromptFields(doc: any) {
  if (!doc || typeof doc !== "object" || !Array.isArray(doc.content)) {
    return { promptInput: "", promptResult: "" };
  }

  // 1. 모든 paragraph 노드 추출
  const paras = doc.content.filter(
  ...
  // 2. 텍스트 변환 헬퍼
  const getText = (para: any) => {
    if (!para || !para.content) return "";
    return para.content.map((c: any) => c.text || "").join("").trim();
  };

  // 3. 순회하며 라벨 찾기
  for (let i = 0; i < paras.length; i++) {
    const text = getText(paras[i]);

    if (text === "PromptInput") {
      // 라벨을 찾으면 바로 다음 문단이 값 (범위 체크 포함)
      if (i + 1 < paras.length) {
        promptInput = getText(paras[i + 1]);
      }
    } else if (text === "PromptResult" || text === "Result") {
      // 'PromptResult' 혹은 이미지 모드일 때의 'Result' 라벨 처리
      if (i + 1 < paras.length) {
        promptResult = getText(paras[i + 1]);
      }
    }
  }
